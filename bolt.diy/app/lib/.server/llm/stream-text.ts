...
export async function streamText(props: {
  ...
  if (chatMode === 'build' && dataContext && Object.keys(dataContext.cubeData).length > 0) {
    logger.info(
      `DATA CONTEXT INJECTED: package="${dataContext.packageName}" (ID: ${dataContext.packageId}), cubes: ${Object.keys(dataContext.cubeData).join(', ')}, total rows: ${Object.values(dataContext.cubeData).reduce((sum, rows) => sum + (rows as unknown[]).length, 0)}`,
    );
    const MAX_INLINE_ROWS = 50;
    const cubeEntries = Object.entries(dataContext.cubeData);

    const cubeDescriptions = cubeEntries
      .map(([cubeName, rows]) => {
        const rowArray = rows as Record<string, unknown>[];
        const fields = rowArray.length > 0 ? Object.keys(rowArray[0]) : [];
        const sample = rowArray.slice(0, 3);
        return `### Cube: ${cubeName}
Fields: ${fields.join(', ')}
Total rows: ${rowArray.length}
Sample (first 3):
${JSON.stringify(sample, null, 2)}`;
      })
      .join('\n\n');

    // For small datasets, include full data inline so the AI can embed it directly
    const totalRows = cubeEntries.reduce((sum, [, rows]) => sum + (rows as unknown[]).length, 0);
    const fullDataBlock =
      totalRows <= MAX_INLINE_ROWS
        ? `\n\nFull dataset (embed this in generated code as JSON constants):\n${JSON.stringify(dataContext.cubeData, null, 2)}`
        : `\n\nThe dataset has ${totalRows} rows total. Create a src/data/ folder with one JSON file per cube containing the full data. The full data per cube:\n${cubeEntries.map(([name, rows]) => `${name}: ${JSON.stringify(rows)}`).join('\n')}`;

    systemPrompt = `${systemPrompt}

<data_context>
  Package: "${dataContext.packageName}" (ID: ${dataContext.packageId})

${cubeDescriptions}
${fullDataBlock}

  INSTRUCTIONS:
  - Generate a React + Vite app that visualizes ALL cubes above.
  - Use Recharts for charts.
  - Each cube needs at least one visualization (chart, table, or both).
  - AI decides layout (dashboard, tabs, etc.) based on data and user description.
